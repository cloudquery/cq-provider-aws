...
CREATE TABLE IF NOT EXISTS "aws_ssm_associations" (
	...
	"targets" jsonb,
	CONSTRAINT aws_ssm_associations_pk PRIMARY KEY(arn),
	UNIQUE(cq_id)
);
CREATE TABLE IF NOT EXISTS "aws_ssm_association_target_locations" (
	...
	CONSTRAINT aws_ssm_association_target_locations_pk PRIMARY KEY(cq_id),
	UNIQUE(cq_id),
	FOREIGN KEY (association_cq_id) REFERENCES aws_ssm_associations(cq_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "aws_ssm_instance_patches" (
	...
	"title" text,
	"cve_ids" text,
	CONSTRAINT aws_ssm_instance_patches_pk PRIMARY KEY(instance_cq_id,kb_id),
	UNIQUE(cq_id),
...
CREATE TABLE IF NOT EXISTS "aws_ssm_patch_baselines" (
	...
	"tags" jsonb,
	CONSTRAINT aws_ssm_patch_baselines_pk PRIMARY KEY(arn),
	UNIQUE(cq_id)
);
CREATE TABLE IF NOT EXISTS "aws_ssm_patch_baseline_approval_rules" (
	...
	CONSTRAINT aws_ssm_patch_baseline_approval_rules_pk PRIMARY KEY(cq_id),
	UNIQUE(cq_id),
	FOREIGN KEY (patch_baseline_cq_id) REFERENCES aws_ssm_patch_baselines(cq_id) ON DELETE CASCADE
);
CREATE TABLE IF NOT EXISTS "aws_ssm_patch_baseline_sources" (
	...
	"name" text,
	"products" text[],
	CONSTRAINT aws_ssm_patch_baseline_sources_pk PRIMARY KEY(cq_id),
	UNIQUE(cq_id),
	FOREIGN KEY (patch_baseline_cq_id) REFERENCES aws_ssm_patch_baselines(cq_id) ON DELETE CASCADE
);
