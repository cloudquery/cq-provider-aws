...
func buildJobsMock(t *testing.T, ctrl *gomock.Controller) client.Services {
	...
	faker.FakeDataSkipFields(&node, []string{"ApplyMapping", "JDBCConnectorSource"})
	job := types.Job{
		CodeGenConfigurationNodes: map[string]types.CodeGenConfigurationNode{"test": node},
	}
	faker.FakeDataSkipFields(&job, []string{"WorkerType", "CodeGenConfigurationNodes"})
	//require.NoError(t,faker.FakeData(&job) )
