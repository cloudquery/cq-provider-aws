resource "random_password" "redshift_password" {
  length           = 16
  special          = true
  override_special = "_%@"
}

module "redshift_vpc" {
  source  = "terraform-aws-modules/vpc/aws"
  version = "~> 3.0"

  name = "cq-provider-aws-redshift-vpc"

  cidr = "10.10.0.0/16"

  azs              = ["eu-central-1a", "eu-central-1b", "eu-central-1c"]
  redshift_subnets = ["10.10.41.0/24", "10.10.42.0/24", "10.10.43.0/24"]
}

###########################
# Security group
###########################
module "redshift_sg" {
  source  = "terraform-aws-modules/security-group/aws//modules/redshift"
  version = "~> 4.0"

  name   = "cq-provider-aws-redshift-sg"
  vpc_id = module.redshift_vpc.vpc_id

  # Allow ingress rules to be accessed only within current VPC
  ingress_cidr_blocks = [module.redshift_vpc.vpc_cidr_block]

  # Allow all rules for all protocols
//   egress_rules = ["all-all"]
}

###########
# Redshift
###########
module "redshift" {
  source  = "terraform-aws-modules/redshift/aws"

  cluster_identifier      = "cq-provider-aws"
  cluster_node_type       = "dc2.large"
  cluster_number_of_nodes = 1

  cluster_database_name   = "mydb"
  cluster_master_username = "mydbuser"
  cluster_master_password = random_password.redshift_password.result

  subnets                = module.redshift_vpc.redshift_subnets
  vpc_security_group_ids = [module.redshift_sg.security_group_id]

  #  redshift_subnet_group_name = module.vpc.redshift_subnet_group
}