...
\i sql/queries/autoscaling/autoscaling_groups_elb_check.sql
...
\i sql/queries/codebuild/check_oauth_usage_for_sources.sql

\set check_id 'codebuild.2'
\echo "Executing check codebuild.2"
\i sql/queries/codebuild/check_environment_variables.sql

\set check_id 'config.1'
\echo "Executing check config.1"
\i sql/queries/config/enabled_all_regions.sql

\echo "Creating view_aws_log_metric_filter_and_alarm"
\i sql/views/log_metric_filter_and_alarm.sql

\set check_id 'cloudwatch.1'
\echo "Executing check cloudwatch.1"
\i sql/queries/cloudwatch/alarm_root_account.sql

\set check_id 'dms.1'
\echo "Executing check dms.1"
\i sql/queries/dms/replication_not_public.sql

\echo "Creating view_aws_security_group_ingress_rules"
\i sql/views/security_group_ingress_rules.sql

\set check_id 'ec2.1'
\echo "Executing check ec2.1"
\i sql/queries/ec2/ebs_snapshot_permissions_check.sql

\set check_id 'ec2.2'
\echo "Executing check ec2.2"
\i sql/queries/ec2/default_sg_no_access.sql
...
\i sql/queries/ec2/get_unused_public_ips.sql

\set check_id 'ec2.5'
\echo "Executing check ec2.5"
\i sql/queries/ec2/no_broad_public_ingress_on_port_22.sql
...
\i sql/queries/elb/elbv2_redirect_http_to_https.sql

\set check_id 'elasticsearch.1'
\echo "Executing check elasticsearch.1"
\i sql/queries/elasticsearch/elasticsearch_domains_should_be_in_vpc.sql

\set check_id 'elasticsearch.2'
\echo "Executing check elasticsearch.2"
\i sql/queries/elasticsearch/elasticsearch_domains_should_have_encryption_at_rest_enabled.sql

\set check_id 'guardduty enabled in all enabled regions'
\echo "Executing check guardduty enabled in all enabled regions"
\i sql/queries/guardduty/detector_enabled.sql
...
\i sql/queries/lambda/lambda_function_in_vpc.sql
...
\i sql/queries/s3/publicly_writable_buckets.sql

\set check_id 's3.2'
\echo "Executing check s3.2"
\i sql/queries/s3/publicly_readable_buckets.sql

\set check_id 's3.3'
\echo "Executing check s3.3"
\i sql/queries/s3/s3_cross_region_replication.sql

\set check_id 's3.4'
\echo "Executing check s3.4"
\i sql/queries/s3/s3_server_side_encryption_enabled.sql

\set check_id 's3.5'
\echo "Executing check s3.5"
\i sql/queries/s3/deny_http_requests.sql

\set check_id 's3.6'
\echo "Executing check s3.6"
\i sql/queries/s3/account_level_public_access_blocks.sql

\set check_id 'sagemaker.1'
\echo "Executing check sagemaker.1"
\i sql/queries/sagemaker/sagemaker_notebook_instance_direct_internet_access_disabled.sql

\set check_id 'secretmanager.1'
\echo "Executing check secretmanager.1"
\i sql/queries/secretsmanager/secrets_should_have_automatic_rotation_enabled.sql

\set check_id 'secretmanager.2'
\echo "Executing check secretmanager.2"
\i sql/queries/secretsmanager/secrets_configured_with_automatic_rotation_should_rotate_successfully.sql

\set check_id 'secretmanager.3'
\echo "Executing check secretmanager.3"
\i sql/queries/secretsmanager/remove_unused_secrets_manager_secrets.sql

\set check_id 'secretmanager.4'
\echo "Executing check secretmanager.4"
\i sql/queries/secretsmanager/secrets_should_be_rotated_within_a_specified_number_of_days.sql
...
\i sql/queries/wafv2/wafv2_web_acl_logging_should_be_enabled.sql
